...
def load_dataset(dataset_path: Path) -> tuple[SampleSet, SampleSet, list[str], dict]:
    ...
    for sample in payload["samples"]:
      if sample["split"] == "eval":
        eval_texts.append(sample["context"])
        eval_labels.append(int(sample["label"]))
      else:
        train_texts.append(sample["context"])
...
def compute_max_length(texts: list[str]) -> int:
    ...
    return max(24, min(160, token_count + 2))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and export a tiny Sudoku hard-set PLACE-value transformer."
    )
    parser.add_argument("--dataset", type=Path, default=Path("soduku/training/hard-value-dataset.json"))
    parser.add_argument("--raw-dir", type=Path, default=Path("soduku/training/hard-value-bert"))
    parser.add_argument("--export-dir", type=Path, default=Path("soduku/models/hard-value-bert"))
...
def main() -> None:
    ...
    train_samples, eval_samples, label_names, metadata = load_dataset(dataset_path)

    if not train_samples.texts or not eval_samples.texts:
        raise RuntimeError("Sudoku value dataset must contain both train and eval samples.")
    ...
    print(
        "training on "
        f"{device} with train={len(train_samples.texts)} eval={len(eval_samples.texts)} "
